...
\item Théorème de \bsc{Kleene}, connaître/mentionner les constructions pour la preuve (parmi \bsc{Thompson}, \bsc{Glushkov}, \bsc{Antimirov}, \bsc{McNaughton-Yamada}, \bsc{Brzozoswky-McCluskey}).
\end{itemize}
...
% calcul d'un  automate émondé = polynomial, éxécuter l'automate  
%sur le mot = O(|A|·|w|) pour non dét et O(|A|+ |w|) pour dét, 
% a inclusion b = a inter comp(b) = vide = pspace complet,  automate 
%minimaux égaux = exp car determinisation, 
%inverser états finaux
...
% clôture de relation réflexive transitive, O(Q^2)
\item Avantages/inconvénients des constructions du théorème de  
\bsc{Kleene} ?

% Thompson => O(|E|^2)
% Gluchkov => exactement |E| +1 états et au plus O(|E|^2) transitions
% dérivé d'Antimirov -=> au plus |E| +1 état
% MC Naugthon yamada => proche de Floyd Warshall, dur à dérouler sur 
%un exemple
% BRZOZOWSKIet MCCLUSKEY => plus intuitives, plus aisées en pratique
